...
var unimpliedfunc func(i Instruction, vm api.LuaVM) = nil
...
type opcode struct {
	...
	action func(i Instruction, vm api.LuaVM) //deal with op
}
...
var opcodes = []opcode{
	...
	opcode{0, 1, OpArgU, OpArgU, IABC /* */, "CALL    ", unimpliedfunc}, // R(A), ... ,R(A+C-2) := R(A)(R(A+1), ... ,R(A+B-1))
	opcode{0, 1, OpArgU, OpArgU, IABC /* */, "TAILCALL", unimpliedfunc}, // return R(A)(R(A+1), ... ,R(A+B-1))
	opcode{0, 0, OpArgU, OpArgN, IABC /* */, "RETURN  ", unimpliedfunc}, // return R(A), ... ,R(A+B-2)
